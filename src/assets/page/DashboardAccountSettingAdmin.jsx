...
function DashboardAccountSetting(){
  ...
  async function manageUser() {
    try {
      const dataUser = await fetch("http://localhost:8100/user/admin/manage");
  ...
  async function searchUser(data) {
    ...
    try {
      const dataUser = await fetch(`http://localhost:8100/user/admin/manage/search?fullname=${search}`);
      const listData = await dataUser.json();
      setUser(Array.isArray(listData.result) ? listData.result : []);
    } catch (error) {
